        ...
        <div className="udapp_contractActionsContainer">
          ...
          {contractABI &&
            contractABI.map((funcABI, index) => {
              ...
              const reencryptInputs = funcABI.inputs.reduce(
                (reencrypt, inp, index) => {
                  if (reencrypt.publicKey && reencrypt.signature)
                    return reencrypt

                  if (
                    inp.type === 'bytes32' &&
                    publicKeyLabels.includes(inp.name)
                  ) {
                    reencrypt.publicKey = index
                  }
                  if (
                    inp.type === 'bytes' &&
                    signatureLabels.includes(inp.name)
                  ) {
                    reencrypt.signature = index
                  }

                  return reencrypt
                },
                {publicKey: null, signature: null} as {
                          ...
                          (key) => {
                            ...
                            return key === funcIndex
                              ? Object.keys(response || {}).map(
                                (innerkey, index) => {
                                  if (
                                    reencryptInputs.publicKey != null &&
                                      reencryptInputs.signature != null
                                  ) {
                                    const [, ciphertext] =
